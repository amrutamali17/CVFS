//Customised Dynamic File System project.
...
void main(char *name)
{
    ...
    {
        printf("Description : Used to display information of file\n");
        printf("Usage : stat Filee_Descriptor\n");
...
void DisplayHelp()
{
    ...
    printf("read : To Read the conttents from file\n");
...
int GetFDFromName(char *name)
{
    ...
    while(i<50)
    {
        if(UFDTArr[i].ptrfiletable != NULL)
           if(strcmp((UFDTArr[i].ptrfiletable->ptrinonde->FileName),name)==0)
           break;
        i++;
...
int CreateFile(char *name,int permission)
{
    ...
    UFDTArr[i].ptrfiletable->coount = 1;
...
int ReadFile(int fd,char *arr, int isize)
{
    ...
    else 
    {
       strncpy(arr,(UFDTArr[fd].ptrfiletable->ptrinode->Buffer)+(UFDTArr[fd].ptrfiletable->readoffest),isize);
        (UFDTArr[fd].ptrfiletable->readoffset) = (UFDTArr[fd].ptrfiletable->readoffset)+isisze;
...
int LseekFile(int fd,int size,int from)
{
    ...
    if((UFDTArr[fd].ptrfiletable->mode == READ) || (UFDTArr[fd].ptrfiletable->mode == READ+WRITE))
    {
        if(from == CURRENT)
        {
            if(((UFDTArr[fd].ptrfiletable->readofffset)+size)> UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize)  return-1;
        ...
        {
            if(size > (UFDTArr[fd].ptrfiletable->pyrinode->FileActualSize))  return -1;
            if(size < 0)  return -1;
...
int fstat_file(int fd)
{
    ...
    if(temp->permission == 1)
       printf("Fille permission : Read only\n");
...
int truncate_File(char *name)
{
    ...
    memset(USFDTArr[fd].ptrfiletable->ptrinode->Buffer,0,1024);
    UFDTArr[fd].ptrfiletable->readoffset = 0;
    UFDTArr[fd].ptrfiletable->writeoffset = 0;
...
int main()
{
    ...
    while(1)
    {
        ...
        if(count == 1)
        {
            ...
            {
                printf("Treminating the Marvellous Virtual File System\n");
                break;
            }
            else
            {
